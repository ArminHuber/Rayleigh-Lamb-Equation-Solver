...
% amplitudes are scaled to a colormap. Modal solutions are represented by
% minima, i.e., by bluish color.
...
k2 = (AngularFrequency./(repmat(PhaseVelocity',1,length(AngularFrequency)))).^2; % wavenumber^2 of Lamb waves
kL2 = repmat((AngularFrequency/Material.LongitudinalVelocity).^2,length(PhaseVelocity),1); % wavenumber^2 of longitudinal bulk waves
kT2 = repmat((AngularFrequency/Material.TransverseVelocity).^2,length(PhaseVelocity),1); % wavenumber^2 of transverse bulk waves
...
a3 = tan(x*Half);
a4 = tan(y*Half);
S = abs(a1./a3./y+a2./a4); % Rayleigh-Lamb equation for symmetric modes (absolute value)
A = abs(a1.*a3./y+a2.*a4); % Rayleigh-Lamb equation for antisymmetric modes (absolute value)

%% dispersion diagrams
f = figure('Name','Symmetric mode dispersion diagram','Units','normalized','Toolbar','none','OuterPosition',[0 0 1 1],'color','w'); %#ok<*UNRCH> 
datacursormode on
...
function output_txt = MyCursor(~,event_obj) %#ok<*DEFNU> 
